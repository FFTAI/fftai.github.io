...
------------------------------------------------------------------------------------------------------------------------------------
auth                             bosdyn.api.AuthService                            auth.RoCS.robot
directory                        bosdyn.api.DirectoryService                       api.RoCS.robot                              user
directory-registration           bosdyn.api.DirectoryRegistrationService           api.RoCS.robot                              user
estop                            bosdyn.api.EstopService                           estop.RoCS.robot                            user
graph-nav-service                bosdyn.api.graph_nav.GraphNavService              graph-nav.RoCS.robot                        user
image                            bosdyn.api.ImageService                           api.RoCS.robot                              user
lease                            bosdyn.api.LeaseService                           api.RoCS.robot                              user
...
```
...
```python
>>> sdk = bosdyn.client.create_standard_sdk('understanding-RoCS')
```

### Create a robot object

To retrieve the robot id like we did in [RoCS Python SDK Quickstart](./quickstart.md) we'll first need to create a `robot` object, using its network address as an argument. In this example, we only create one `robot` object, but it is possible to create and control multiple robots in the same program with the Boston Dynamics API.
...
As discussed earlier, RoCS exposes its capability via a number of services. The Boston Dynamics Python API has a corresponding set of clients for each service, which are created off of the robot object.
...
Traceback (most recent call last):
  File "/mnt/c/RoCS_v2_0/bblank_RoCS_v2_0_env/lib/python3.6/site-packages/bosdyn/client/common.py", line 290, in call
...
We encourage you to experiment with these various parameters, referencing the [robot_command proto](../../protos/bosdyn/api/robot_command.proto) parent class for general Boston Dynamics robots and the [robot command proto](../../protos/bosdyn/api/RoCS/robot_command.proto) RoCS subclass.
